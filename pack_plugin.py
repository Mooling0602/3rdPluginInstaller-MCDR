...
def main():
    ...
    if build_plg == 1:
        if is_stable == 1:
            output_name = f"3rdPluginInstaller-v{main_ver}-{framework_ver}.mcdr"
            files_to_add = ["mcdreforged.plugin.json", "requirements.txt", "LICENSE"]
            dirs_to_add = ["lang", "third_party_plg_installer"]
        else:
            output_name = f"3rdPluginInstaller-v{main_ver}.mcdr"
